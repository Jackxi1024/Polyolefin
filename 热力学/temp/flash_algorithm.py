...
import pcsaft
import sys
sys.path.append('E:\\AZFT\\polymer_model')
...
def VLE_TPflash(T, P, z, args, max_iter=100):
    """
    Two-Phase PT Flash by Successive substitution and the Rachford-Rice equation
    """

    # Tb = bubble_temperature(P, z, args)[0]  # 泡点温度
    # Td = dew_temperature(P, z, args)[0]     # 露点温度
    # 过冷液体
    # if T < Tb:   
    #     return 0, z, None, 0 
    # # 过热气体
    ...
    # STABILITY ANALYSIS
    stability = 1
    # tm_l, tm_v, fugcoef_l, fugcoef_v, fugcoef_zl, fugcoef_zv = stability_analysis(T,P,z,K,args)
    # if  not (abs(tm_v)<1e-5 or abs(tm_l)<1e-5) and abs(tm_l-tm_v)>1e-5:
    #     # if any tm<0 then we are in the two-phase region
    #     if tm_l<0 and tm_v<0:
    #         K = fugcoef_l/fugcoef_v
    #     elif tm_l<0:
    #         K = fugcoef_l/fugcoef_zv
    #     elif tm_v<0:
    #         K = fugcoef_zl/fugcoef_v
    #     else:
    ...
    while (delta_K>1e-6) & (iter_total<max_iter):

        # 逃出K值的死循环
        # if iter_total == max_iter/2 and max(abs(K-K_array[iter_total-1,:]))>1e-7 :
        #     pass
        
        if stability == 1:
            beta, x, y = Rachford_Rice_solver(K,z)        # Call to RR_solver with actual K's
        else:
            beta, x, y = Rachford_Rice_solver(K,z,1)
...
def constraints(k, z):
    ...
    error = np.sum(error) + np.abs(np.sum(x)-1) + np.abs(np.sum(y)-1)
...
def gibbs_minization(T, P, z, args):
    ...
    x0 = [0.5, 0.5, 0.1, 0.2, 0.02 ,0, 0, 0, 0.8, 0.2, 0, 0, 0, 0, 0]
...
if __name__ == "__main__":
    ...
    beta, x, y, iter = VLE_TPflash(T, P, z, pyargs)
    print("beta:", beta)
    print("x:", x)
    print("y:", y)
    print("iter:", iter)
