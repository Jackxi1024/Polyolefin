...
tol = 0.0001
error = 2*tol
delt = np.zeros(NC+2)   # 撕裂流股的差值: 摩尔焓、压力、各组分流量
iter = 0
while True:
    iter = iter+1
    ...
    print("iter: ", iter)
    print("cycle_in: \n", cycle_in)
    H_in = molar_enthalpy_of_mixture(components, args, cycle_in)[1]
    ...
    cycle_out = hot_out[0]
    print("cycle_out: \n", cycle_out)
    H_out = molar_enthalpy_of_mixture(components, args, cycle_out)[1]

    # 计算偏差
    delt[0] = (H_out - H_in)/H_in
    delt[1] = (cycle_out["P"] - cycle_in["P"])/cycle_in["P"]
    F_in = cycle_in["Mole Flow"]
    F_out = cycle_out["Mole Flow"]
    z_in = np.array(list(cycle_in["Mole Fraction"].values())) 
    z_out = np.array(list(cycle_out["Mole Fraction"].values()))
    Fc_in = F_in*z_in
    Fc_out = F_out*z_out
    for i in range(NC):
        if Fc_in[i] != 0:
            delt[2+i] = (Fc_out[i]-Fc_in[i])/Fc_in[i]
        else:
            delt[2+i] = Fc_out[i]
    
    error = np.max(np.abs(delt))
    print("error: ", error)
    if (error <= tol):
        break
    else:
