...
def bubble_pressure(T, x, args, tol=1e-7, max_iter=100):
    """
    ...
    if iter_inner >= max_iter or iter >= max_iter:
        state = False
    if len(x)>1 and abs(rho_l-rho_v) < 0.1:
        state = False
    return P, y, state

def bubble_temperature(P, x, args, tol=1e-7, max_iter=100):
    """
...
def dew_pressure(T, y, args, tol=1e-7, max_iter=100):
    """"
...
def dew_temperature(P, y, args, tol=1e-7, max_iter=100):
    """
...
def TP_flash(T, P, z, args, tol=1e-7, max_iter=100, valid_phases="vapor-liquid"):
    """ 先计算泡露点, 判断是否处于两相区, 再考虑闪蒸 """

    Pb, _, state1 = bubble_pressure(T, z, pyargs)     # 泡点压力
    Tb, _, state2 = bubble_temperature(P, z, pyargs)  # 泡点温度
    Pd, _, state3 = dew_pressure(T, z, pyargs)        # 露点压力
    Td, _, state4 = dew_temperature(P, z, pyargs)     # 露点温度
    iter = 0

    # 考虑纯物质的情况(P=Pb=Pd)
    if abs(Pb-Pd)<0.1 and abs(P-Pb)<0.1:
        if valid_phases == "vapor":
            beta = 1
        elif valid_phases == "liquid":
            beta = 0
        x = z
        y = z
    else:
        ...
        # 低于露点压力，处于气相
            ...
            beta, x, y, iter = VLE_TPflash(T, P, z, args, tol, max_iter)
...
def VLE_TPflash(T, P, z, args, tol=1e-7, max_iter=100):
    ...
    """
    beta, x, y, iter = VLE_TPflash_first(T, P, z, args, tol, max_iter)
    if not(beta>=0 or beta<=1) or iter == max_iter:
        beta, x, y, iter = VLE_TPflash_second(T, P, z, args, tol, max_iter)
    return beta, x, y, iter


def VLE_TPflash_first(T, P, z, args, tol=1e-7, max_iter=100):
    """
...
def VLE_TPflash_second(T, P, z, args, tol=1e-7, max_iter=100):
    """
    Two-Phase PT Flash by Successive substitution and the Rachford-Rice equation
...
if __name__ == "__main__":
    ...
    beta, x, y, iter = VLE_TPflash_first(T, P, z, pyargs)
    print("beta:", beta)
    print(iter)
    ...
    s = np.array([2.9729, ])
    e = np.array([12.53, ])
    pcsaft_args = {"m":m, "s":s, "e":e}
    print(bubble_pressure(T, z, pyargs))
    print(bubble_temperature(P, z, pyargs))
    print(dew_pressure(T, z, pyargs))
    print(dew_temperature(P, z, pyargs))
    ...
    beta, x, y, iter = VLE_TPflash(T, P, z, pyargs, max_iter=300)
    x[4] = x[4]*DPN
    beta = beta/(beta+(1-beta)*sum(x))
    print("新气相分数：", beta)
    x = x/sum(x)
